    ...
    public class BeanVarietyRepository : IBeanVarietyRepository
    {
        ...
        public List<BeanVariety> GetAll()
        {
            using (SqlConnection conn = Connection)
            {
                ...
                {
                    cmd.CommandText = "SELECT Id, [Name], Region, Notes FROM dbo.BeanVariety";

                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        List<BeanVariety> varieties = new();

                        while (reader.Read())
                        {
                            BeanVariety newVariety = new()
                            {
                            ...
                            {
                                newVariety.Notes = reader.GetString(reader.GetOrdinal("Notes"));
                            }
